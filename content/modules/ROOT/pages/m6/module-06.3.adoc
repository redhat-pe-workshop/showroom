...
How about we added a check if the commit had been signed? We don't want to break the build necessarily, but here's the thing:

* if we are lenient here and tolerate unsigned commits, these commits are part of the commit history.
...
As mentioned above, the pipeline is provisioned by the "*Parasol Store Development*" template - since it is our responsibility as Platform Engineers to continuously evolve those templates based on developer requirements and company policies - we'll change the template:
