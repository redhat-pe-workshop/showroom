...
1) We'll change the pipeline that is rolled out via the *Parasol Store Development* Template to include steps for commit verification

2) We'll use the branch with our signed commits that we created earlier and instantiate a new development environment with that updated template
...
=== 1) PE: Updating the template

. The template that we imported (remember the step "*Register Existing Component*" in xref:m4/module-04.4.adoc[Chapter 4.4]?) is located https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/parasol-store-dev-template[here^, window="gitlab"], with the `template.yaml` linking all the bits & pieces together. Feel free to look around, if you haven't already.

==== *1.1) Pipeline Definition*

. We'll dive right into the pipeline definition: Click https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/parasol-store-dev-template/-/edit/main/manifests/helm/parasol-store-build/templates/pipeline-build.yaml?ref_type=heads[here to edit it^, window="gitlab"]

. There, you will find a section under `tasks` that is commented. Please uncomment the whole section:

image:m6/template_enable_tasks.png[]

A little further below, in the `build-source` task, please uncomment the `- verify-commit` and comment the `- git-clone`. 

image:m6/template_enable_sequence.png[]

As usual with `yaml` make sure to double-check the indentation... 

[cols="1,1", frame=none, grid=none]
|===
| image:m6/template_enabled.png[]| image:m6/indentationaccident.jpeg[width=50%] 
| image:m6/template_enabled_sequence.png[]| |
|===

...and give it a meaningful commit message.

==== *1.2) TriggerTemplate Definition*

We have a task (`setup-gitsign-environment`) that needs to gather the TAS (Trusted Artifact Signer) endpoints in the TAS namespace - therefore it needs reader access to that namespace, but the default `pipeline` serviceAccount doesn't have that. We created a serviceAccount with `clusterReader` access, but we don't want to extend that elevated privilege to the whole pipeline. We just assign it to this one task - all others will run with the default access levels.

Click https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/parasol-store-dev-template/-/edit/main/manifests/helm/parasol-store-build/templates/triggertemplate-build.yaml?ref_type=heads[here to edit^, window="gitlab"] the TriggerTemplate:

image:m6/triggertemplate-uncomment.png[]

This is what it should look like:

image:m6/triggertemplate-uncommented.png[]

Now commit it.

What have done now? We have modified the *helm templates* (`parasol-store-dev-template ➡️ manifests ➡️ helm ➡️ parasol-store-build ➡️ templates ➡️ triggertemplate-build.yaml / pipeline-build.yaml` ) that are instantiated when we use the template, as defined in the step `template-gitops-deployment` in the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/parasol-store-dev-template/-/blob/main/template.yaml#L62[template.yaml^, window="gitlab"]. 

image:m6/parasol-store-development-template.png[]

IMPORTANT: It's important to understand that a change to a template will *not* change any code, configuration or deployment artifacts of a component that has already been created with an earlier version of the template. That is by design - we don't want to change anything our developers are already using. We could have created a new template instead of changing an existing one. How you want to implement that in _your_ environment is again completely up to you, as it depends on your requirements and company policies for developers.

=== 2) DEV: Using the updated template
...
Since we already created a `parasol-store` branch (`new-feature-with-signed-commits`) above via commandline and already pushed a signed commit to it - we'll simply use that.

NOTE: For details what we're doing and why - go back to xref:m4/module-04.5.adoc[Chapter 4.5] for a detailed explanation. We're now setting up a developer environment, but with the updated template.
...
* Click "Choose" on the "Parasol Store Development" template
...
After a few moments, the ArgoCD job will kick off the first pipeline run (`*-init-*`) in your new environment. However, this *fails* because the PipelineRun in the job definition doesn't have the additional serviceAccount that is needed to get all the endpoints.

image:m6/init-pipeline-failed.png[]
image:m6/init-pipeline-failed-log.png[]

NOTE: This is fine, because we're checking if the "pusher" has also signed the commit. When a job triggers the pipeline, there is no `git push` or `git commit` to trigger the pipeline, therefore also no "pusher". This is just one way to implement it - you can verify a number of attributes. If that failed run bothers you, you can go and delete the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/parasol-store-dev-template/-/blob/main/manifests/helm/parasol-store-build/templates/job-trigger-pipeline-run.yaml[job^, window="gitlab"] from the template.
...
NOTE: Remember, here we're not checking if the git author signed the commit, but we're checking if the person pushing signed it. However, that is completely up to you. We can also check the "claims" (user and OIDC issuer) of the signature with regular expressions, adding flexibility to your verification. If you want to inspect how this is done, take a look at the https://gitlab-gitlab.{openshift_cluster_ingress_domain}/rhdh/parasol-store-dev-template/-/blob/main/manifests/helm/parasol-store-build/templates/task-verify-commit.yaml[task-verify-commit.yaml^, window="gitlab"] task. It basically comes down to this one line: `gitsign verify --certificate-identity $(params.email) --certificate-oidc-issuer $OIDC_ISSUER_URL HEAD` where we can change to `--certificate-identity-regexp` and/or `--certificate-oidc-issuer-regexp`, respectively.
